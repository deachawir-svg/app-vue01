<template>
      ...
      <tbody>
        ...
        <span v-if="employee.active == 1">ปกติ</span>
          <span v-else>ลาออก</span>
            ...
            <img
              v-if="employee.image"
              :src="'http://localhost/APP-VUE01/php.api/image/' + employee.image"
              width="100"
            />
            </td>
  ...
  <div v-if="isEditMode && editForm.image">
    ...
    <img
      :src="'http://localhost/APP-VUE01/php.api/image/' + editForm.image"
      width="100"
    />
  </div>
</div>
...
export default {
  ...
  setup() {
    ...
    const fetchemployees = async () => {
      try {
        const res = await fetch("http://localhost/app-vue01/php.api/api_employee.php");
